...
struct Knight : BaseFigure {
  std::vector<Point> threat(const Point &p) {
    ...
    for (int i = 0; i < 8; ++i) {
      for (int j = 0; j < 8; ++j) {
        if (std::abs(p.x - i) == 1 && std::abs(p.y - j) == 2 ||
            std::abs(p.x - i) == 2 && std::abs(p.y - j) == 1) {
...
struct Bishop : BaseFigure {
  std::vector<Point> threat(const Point &p) {
    ...
    for (int i = 0; i < 8; ++i) {
      for (int j = 0; j < 8; ++j) {
        if (std::abs(p.x - i) == std::abs(p.y - j) && i != p.x) {
          answer.push_back(Point(i, j));
        }
      }
    }
...
struct King : BaseFigure {
  std::vector<Point> threat(const Point &p) {
    ...
    for (int i = 0; i < 8; ++i) {
      for (int j = 0; j < 8; ++j) {
        if (std::abs(p.x - i) <= 1 && std::abs(p.y - j) <= 1 &&
            Point(i, j) != p) {
          answer.push_back(Point(i, j));
        }
      }
...
struct Pawn : BaseFigure {
  std::vector<Point> threat(const Point &p) {
    ...
    for (int i = 0; i < 8; ++i) {
      for (int j = 0; j < 8; ++j) {
        if (std::abs(p.x - i) == 1 && (p.y + 1 == j || p.y - 1 == j)) {
          answer.push_back(Point(i, j));
        }
      }
    }
